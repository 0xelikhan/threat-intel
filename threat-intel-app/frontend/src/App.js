  ...
  return (
    <Box sx={{
      ...
      <Sidebar
        ...
        onOpenAnalyze={() => {
          // Dismiss the email composer / scanner so the analyze view
          // owns the main area again. The analysis result itself is
          // preserved (it lives in `result`) so we just need to clear
          // the overlays.
          setEmailState(null);
          clearScan();
        }}
      ...
      {/* Main view priority: email composer > file scanner > analysis.
          (Settings UI removed; operators edit data/config.json directly
          or via /api/settings.) */}
      {emailState && (

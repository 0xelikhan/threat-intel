...
def test_oversize_content_length_rejected_with_envelope(client):
    """Setting Content-Length to 60 MB (above the 50 MB cap) must be
    ...
    # Don't actually send 60 MB; the middleware reads Content-Length
    ...
    assert r.status_code == 413, f"60 MB Content-Length was NOT rejected: {r.status_code} {r.text}"
    body = r.json()
...
def test_boundary_at_cap_is_accepted(client):
    """A POST at exactly the cap boundary should still pass through. The
    middleware uses `>` not `>=`, so 50 MB exact is allowed."""
    ...
    # The actual handler will reject for missing URLSCAN_KEY (400) or
    # parse the body and accept (200). The point is: NOT 413.
    assert r.status_code != 413, (
        f"body exactly at the cap was wrongly rejected: {r.status_code} {r.text}"
    )

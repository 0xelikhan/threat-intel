...
async def run_triage(state: dict, defer_ai: bool = False) -> dict:
    ...
    try:
        ...
        _bh_patterns = [
            ...
            (r"\bkerberoast(?:ing)?\b", 0.55),
            (r"\bAS[-_ ]REP\s+roast(?:ing)?\b", 0.55),
            (r"\bgolden\s+ticket\b|\bsilver\s+ticket\b", 0.55),
            (r"\bDCSync\b", 0.55),
            (r"\bzerologon\b|\bCVE-2020-1472\b", 0.55),
            (r"\bprintnightmare\b|\bCVE-2021-34527\b", 0.55),
            (r"\bfollina\b|\bms-msdt\b", 0.50),
            (r"\$\{jndi\s*:", 0.55),
            ...
            (r"\bmanage-bde\s+.*?-disable", 0.50),
            (r"\bSuspend-BitLocker\b", 0.45),
            # Windows persistence
            (r"__InstanceModificationEvent\s+.*?WITHIN", 0.45),
            (r"schtasks(?:\.exe)?\s+/create\s+.*?/sc\s+(?:onlogon|onstart)", 0.40),
            (r"HK(?:LM|CU)\\Software\\Microsoft\\Windows\\CurrentVersion\\Run.*?(?:AppData|Public|Temp|ProgramData)", 0.40),
            # Cloud additions
            (r"GetSecretValue\s+.*?in\s+\d+\s+min|Secrets\s+accessed\s*:\s*\d{2,}", 0.50),
            (r"Conditional\s+Access\s+policy\s+disabled", 0.50),
            (r"roles?/(?:owner|editor|iam\.securityAdmin|iam\.roleAdmin)\s+granted", 0.50),
            (r"O365.*?forwarding\s+rule.*?forward\s+to\s*:\s*.*?@", 0.45),
            (r"Delete\s+on\s+forward\s*:\s*true", 0.45),
            # Linux additions
            (r"authorized_keys\b.*?(?:new\s+ssh\s+key|added)", 0.45),
            (r"pam_permit\.so", 0.55),
            (r"/etc/ld\.so\.preload", 0.50),
            (r"NOPASSWD.*>>\s*/etc/sudoers|echo\s+.*?NOPASSWD.*?>>\s*/etc/sudoers", 0.50),
            (r"~?/\.(?:bashrc|profile|zshrc)\b.*?(?:curl|wget|nc\s+-e)", 0.45),
            (r"\bjournalctl\s+--vacuum-(?:time|size|files)\s*[=\s]", 0.50),
            (r"rsyslog\.conf.*?\*\.\*\s+@@?", 0.50),
            (r"\bmeterpreter\b", 0.55),
            (r"\bmetasploit\b\s+.*?(?:handler|payload|reverse)", 0.55),
            (r"\bfind\s+\S+.*?-exec\s+.*?(?:sh|bash)\s+-p", 0.50),
            (r"docker\s+run\s+.*?--cap-add\s*=?\s*ALL", 0.50),
            (r"docker\s+run\s+.*?-v\s+/:/host", 0.55),
            (r"docker\s+run\s+.*?--(?:pid|network|ipc|uts)=host", 0.45),
            (r"iptables\s+.*?-[AI]\s+(?:OUTPUT|FORWARD).*?-j\s+ACCEPT", 0.50),
            (r"ssh-keyscan\s+.*?(?:for\s+\S+\s+in|>>\s+/tmp/)", 0.45),
            (r"/etc/modules-load\.d/\S+\.conf\b.*?(?:created|written|registered|modified|added)", 0.45),
            # macOS additions
            (r"\bDYLD_INSERT_LIBRARIES\s*=", 0.55),
            (r"\bfdesetup\s+disable\b", 0.55),
            (r"/etc/periodic/.*?(?:modified|written).*?(?:curl|wget|osascript)", 0.50),
            (r"/Library/LaunchDaemons/[a-z0-9_.\-]+\.plist\b.*?(?:ProgramArguments\s+changed|replaced)", 0.50),
            (r"~?/?Library/Preferences/com\.apple\.loginitems\.plist.*?(?:new\s+entry|added)", 0.40),
            (r"authorizationdb\s+write\b.*?system\.privilege\.admin", 0.55),
            (r"ES_EVENT_TYPE_NOTIFY_EXEC.*?/Downloads/.*?[Ss]igned\s*:\s*no", 0.45),
            (r"ES_EVENT_TYPE_NOTIFY_EXEC.*?/(?:tmp|private/tmp)/", 0.40),
        ]

...
Adds to the IOC enrichment pipeline:
  ...
  * IntelX — dark web + paste site index (domains / IPs / emails)
...
async def intelx_search(session, term: str, intelx_key: Optional[str]) -> Dict[str, Any]:
    ...
    if not isinstance(init, dict) or "error" in init:
        return {"source": "intelx", "error":
                (init.get("error") if isinstance(init, dict) else "init failed"),
                "error_type": init.get("error_type", "unreachable")
                              if isinstance(init, dict) else "unreachable"}
    search_id = init.get("id")
    if not search_id:
...
def _parse_intelx(r: Any, term: str) -> Dict[str, Any]:
    if isinstance(r, dict) and "error" in r:
        return {"source": "intelx", "error": r.get("error"),
                "error_type": r.get("error_type", "unreachable")}
    if not isinstance(r, dict):
